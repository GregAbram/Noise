...
double noise::GradientCoherentNoise4D (double x, double y, double z, double t, int seed,
  NoiseQuality noiseQuality)
{
  // Create a unit-length cube aligned along an integer boundary.  This cube
  // surrounds the input point.
  ...
  switch (noiseQuality) {
    ...
    case QUALITY_STD:
      ...
      ys = SCurve3 (y - (double)y0);
      zs = SCurve3 (z - (double)z0);
      ts = SCurve3 (t - (double)t0);
      break;
    case QUALITY_BEST:
      xs = SCurve5 (x - (double)x0);
      ys = SCurve5 (y - (double)y0);
      zs = SCurve5 (z - (double)z0);
      ts = SCurve5 (t - (double)t0);
      break;
  }
  ...
  ix1  = LinearInterp (n0, n1, xs);
  iy1  = LinearInterp (ix0, ix1, ys);
  double iz0 = LinearInterp (iy0, iy1, zs);
  ...
  ix1  = LinearInterp (n0, n1, xs);
  iy1  = LinearInterp (ix0, ix1, ys);
  double iz1 = LinearInterp (iy0, iy1, zs);

  return LinearInterp(iz0, iz1, ts);

}

double noise::GradientNoise4D (double fx, double fy, double fz, double ft, int ix,
  int iy, int iz, int it, int seed)
{
  // Randomly generate a gradient vector given the integer coordinates of the
  // input value.  This implementation generates a random number and uses it
  // as an index into a normalized-vector lookup table.
  ...
  double yvGradient = g_randomVectors[(vectorIndex << 2) + 1];
  double zvGradient = g_randomVectors[(vectorIndex << 2) + 2];
  double tvGradient = g_randomVectors[(vectorIndex << 2) + 3];
...
int noise::IntValueNoise4D (int x, int y, int z, int t, int seed)
{
  // All constants are primes and must remain prime in order for this noise
  // function to work correctly.
  int n = (
...
double noise::ValueCoherentNoise4D (double x, double y, double z, double t, int seed,
  NoiseQuality noiseQuality)
{
  // Create a unit-length cube aligned along an integer boundary.  This cube
  // surrounds the input point.
  ...
  n0   = ValueNoise4D (x0, y0, z0, t0, seed);
  n1   = ValueNoise4D (x1, y0, z0, t0, seed);
  ix0  = LinearInterp (n0, n1, xs);
  n0   = ValueNoise4D (x0, y1, z0, t0, seed);
  n1   = ValueNoise4D (x1, y1, z0, t0, seed);
  ix1  = LinearInterp (n0, n1, xs);
  iy0  = LinearInterp (ix0, ix1, ys);
  n0   = ValueNoise4D (x0, y0, z1, t0, seed);
  n1   = ValueNoise4D (x1, y0, z1, t0, seed);
  ix0  = LinearInterp (n0, n1, xs);
  n0   = ValueNoise4D (x0, y1, z1, t0, seed);
  n1   = ValueNoise4D (x1, y1, z1, t0, seed);
  ix1  = LinearInterp (n0, n1, xs);
  iy1  = LinearInterp (ix0, ix1, ys);
  double iz0 = LinearInterp (iy0, iy1, zs);

  n0   = ValueNoise4D (x0, y0, z0, t1, seed);
  n1   = ValueNoise4D (x1, y0, z0, t1, seed);
  ix0  = LinearInterp (n0, n1, xs);
  n0   = ValueNoise4D (x0, y1, z0, t1, seed);
  n1   = ValueNoise4D (x1, y1, z0, t1, seed);
  ix1  = LinearInterp (n0, n1, xs);
  iy0  = LinearInterp (ix0, ix1, ys);
  n0   = ValueNoise4D (x0, y0, z1, t1, seed);
  n1   = ValueNoise4D (x1, y0, z1, t1, seed);
  ix0  = LinearInterp (n0, n1, xs);
  n0   = ValueNoise4D (x0, y1, z1, t1, seed);
  n1   = ValueNoise4D (x1, y1, z1, t1, seed);
  ix1  = LinearInterp (n0, n1, xs);
  iy1  = LinearInterp (ix0, ix1, ys);
  double iz1 = LinearInterp (iy0, iy1, zs);
...
double noise::ValueNoise4D (int x, int y, int z, int t, int seed)
{
  return 1.0 - ((double)IntValueNoise4D (x, y, z, t, seed) / 1073741824.0);
}
